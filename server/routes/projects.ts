...
router.get("/:id/canvas", async (req, res) => {
  try {
    ...
    const canvasData = project.canvasData as { windows?: unknown[] } | null;
    res.json({
...
router.get("/:id/git-status", async (req, res) => {
  try {
    const requestedDirectory = (req.query.directory as string)?.trim() || null;
    ...
    const result = await runGitAction({
      ...
      connectionId:
        typeof body.connectionId === "number" && Number.isFinite(body.connectionId)
          ? body.connectionId
          : null,
      action: body.action as GitAction,
      paths: body.paths,
