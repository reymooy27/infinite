...
function parseLastCommit(output: string) {
  ...
  return { hash, subject, relativeDate, authorName } satisfies GitLastCommit;
}
...
function parseStashes(output: string) {
  return output
    ...
    .map((line) => {
      ...
      return { selector, subject } satisfies GitStashEntry;
    })
...
export async function runGitAction(options: {
  ...
  switch (options.action) {
    ...
    case "discard": {
      ...
      return { ok: true, stdout: "", stderr: "", nextStatus } satisfies GitActionResult;
    }
    case "commit": {
