...
app.post("/api/tunnels", async (req, res) => {
    ...
    const targetHost = typeof req.body?.targetHost === "string" && req.body.targetHost
        ? req.body.targetHost
        : "127.0.0.1";
...
function getUserIdFromRequest(_req) {
    void _req;
    return LOCAL_USER_ID;
}
function getUserIdFromSession(_req) {
    void _req;
    return LOCAL_USER_ID;
}
async function loadConnection(connId, userId) {
    let row;
    try {
...
function proxyThroughAgent(browserWs, agentWs, connection, windowId, initialDirectory) {
    const existing = windowId ? getAgentProxySession(windowId) : undefined;
    const sessionId = existing?.sessionId ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    ...
    if (existing?.onAgentMessage) {
        ...
        onAgentMessage = (raw) => {
            try {
                ...
                if (msg.type === "data") {
                    const payload = msg.encoding === "base64"
...
wss.on("connection", async (ws, req) => {
    ...
    if (pathname === "/ws/sftp") {
        ...
        try {
            ...
            createSFTPConnection(connection, ws);
        }
        catch (err) {
