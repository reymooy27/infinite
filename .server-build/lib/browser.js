...
class BrowserManager {
    ...
    async handleConnection(ws, windowId, width, height) {
        ...
        await page.setUserAgent(await this.browser.userAgent().then((ua) => ua.replace(/\sHeadlessChrome\//, " Chrome/")));
        const session = {
            page,
            client,
            ws,
    ...
    attachListeners(ws, session, windowId) {
        ws.on("message", (data) => {
    ...
    handleClientMessage(session, msg) {
        ...
        switch (msg.type) {
            ...
            case "click": {
                ...
                const clickCount = (Number(msg.clickCount) === 2 ? 2 : 1);
    ...
    async restartScreencast(session) {
